...
import { faArrowDownWideShort, faArrowUpWideShort, faTrashCan } from "@fortawesome/free-solid-svg-icons";
      ...
      <div className="w-full">
        <div className="relative w-full">
          ...
          <button onClick={() => handleVisibility()} className={`
            ...
            <FontAwesomeIcon icon={show ? faArrowUpWideShort : faArrowDownWideShort} size={'lg'} />
          </button>
          <div className={`w-full ${show ? 'border-t border-t-[0.5px] border-t-[grey] border-opacity-20' : ''}`}>
          {
            information.map((info) => {
              const key = Object.keys(info)[0]
              return (
                <div key={key} className={`
                  ${show ? 'h-[81px]' : 'h-[0px]'} ${hide ? 'invisible' : 'visible'}
