...
const NewHeader = () => {
  ...
  useEffect(() => {
    headerState.sideBar || headerState.searchDropdown || headerState.profileDropdown ? setPageActive(false) : setPageActive(true);
  }, [headerState]);

	return (
    <header className={
      `fixed w-full h-[60px] shadow-md z-[99999999999]
      flex flex-row items-center bg-white
      justify-between`
    }>
      <SideBar />
      <a href="/" className={`${headerState.searchActive ? 'hidden' : 'block'} md:block text-midPurple`} >
