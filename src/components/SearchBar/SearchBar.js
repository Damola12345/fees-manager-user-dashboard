...
const SearchBar = (props) => {
	...
	useEffect(() => {
		...
		? 
		setHeaderState({...defaultHeaderState, searchActive: true, searchDropdown: true})
		:
		console.log()
	}, [presentItems]);
	...
	return (
		<div tabIndex={1} className='search relative flex flex-col items-center'>
			<div className='searchBar text-purple flex flex-row md:justify-between items-center gap-2'>
				<FontAwesomeIcon icon={headerState.searchActive ? faXmarkCircle : faMagnifyingGlass} size="lg"
					onClick={() => {
						setHeaderState({...defaultHeaderState, searchActive: !headerState.searchActive});
						setPresentItems([]);
						setinputValue('');
				...
				/>
				<input type="text" placeholder='Search' value={inputValue} onChange={handleChange} className={`
