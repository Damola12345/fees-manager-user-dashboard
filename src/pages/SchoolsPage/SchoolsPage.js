...
const SchoolsPage = () => {
  ...
  useEffect(() => {
    DATABASE === 'BACKEND' && fetch(`${BACKEND_HOST}/schools`, {
      method: "GET",
      headers: {
              ...
              schools.map( (sch) => {
                return (
                  <div key={sch._id} onClick={() => {
                    ...
                    <DisplayCard>
                      <DisplayIcon>
                        <FontAwesomeIcon icon={faPeopleRoof} size='lg' color={midPurple} />
                      </DisplayIcon>
  ...
  return (
    <section className="w-full flex flex-col items-center p-10 py-10 md:px-20">
      ...
      <div className="w-full flex flex-row justify-between items-center bg-[red]">
      </div>
