...
const EditClassroom = (props) => {
  ...
  useEffect(() => {
    fetch(
      `${BACKEND_HOST}/classrooms/${props.id}?schoolName=${localStorage.currentSchool}`,
      {
        method: "GET",
        headers: {
      ...
      <div id="input-single">
        ...
        <div className="edit-input">
          <input
            id="input-teacher"
            type="text"
            defaultValue={classroom.classTeacher}
            name="teacher"
            onChange={(event) => {
      ...
      <div id="input-single">
        ...
        <div className="edit-input">
          <input
            id="input-fees"
            type="number"
            defaultValue={classroom.classFees}
            name="fees"
            onChange={(event) => {
              setClassFees(event.target.value);
              alertMessage();
            }}
          ></input>
        </div>
  ...
  const saveEdit = () => {
    ...
    if (classroom.classTeacher !== classTeacher)
      info.classTeacher = classTeacher;
    ...
    if (Object.keys(info).length > 0) {
      ...
      alertMessage(
        "No changes made. Classroom is up to date!",
        "block",
        "green"
      );
    }
  ...
  const editClass = () => {
    setIsLoading(true);
    fetch(
      `${BACKEND_HOST}/classrooms/${props.id}/edit?schoolName=${localStorage.currentSchool}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
