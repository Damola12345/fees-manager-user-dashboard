      ...
      fetch(`${BACKEND_HOST}/resend-mail`, {
        ...
        .then((response) => {
          if (response.ok) {
            const resentMsg = document.getElementById("resent");
            resentMsg.innerText =
              "Verification code has been sent! Please check your mail";
    ...
    fetch(`${BACKEND_HOST}/verify-email`, {
      ...
      .then((response) => {
        if (response.ok) {
          setTimeout(() => {
            alertMessage(
              'Success! Your email has been verified.<p id="tiny-txt">You will be redirected shortly.</p>',
              "block",
              "green"
            );
            setIsLoading(false);
          ...
          setTimeout(() => {
            setIsLoading(false);
            alertMessage(
              'Wrong code. Please check the code and retry.<p id="tiny-txt">or resend code if expired</p>',
              "block",
              "red"
            );
          }, 3000);
  ...
  const verifyLayout = (
    <div id="contd">
      <p className="verify-text">
        An Email has been sent to <b>{location.state.email}</b>. Kindly check
        your mail and provide the verification code sent to you.
      </p>
      <input
        id="verify-input"
        type="password"
      ...
      <div id="resend-btn" onClick={resetClick}>
        Resend
      </div>
      <div id="resent"></div>
    </div>
